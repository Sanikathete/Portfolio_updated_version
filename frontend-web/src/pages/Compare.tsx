...
const Compare: React.FC = () => {
    ...
    const load = async () => {
      ...
      const portfolioAssets = items.map((item: any, index: number) => {
        ...
        return {
          ...
          pe: Number(stock.pe_ratio ?? 18),
          benefit: 65 + index * 3,
          marketReturn: 8 + index * 2,
          linear: 9 + index * 2,
          arima: 10 + index * 2,
          performance: 12 + index * 2,
        };
      });
      const all = [
        ...portfolioAssets,
        { name: 'Gold', type: 'Commodity', price: 63000, pe: 0, benefit: 78, marketReturn: 8, linear: 9, arima: 11, performance: 14 },
        { name: 'Silver', type: 'Commodity', price: 750, pe: 0, benefit: 74, marketReturn: 10, linear: 11, arima: 12, performance: 16 },
  ...
  return (
    <PageLayout title="Compare">
      <SectionHeader label="Opportunity Matrix" title="Compare" description="Selected portfolio assets plus Gold, Silver, and Bitcoin share a consistent comparison workspace." />
      {!selectedPortfolioId ? <div className="empty-state">Please select a portfolio from Dashboard</div> : (
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div className="glass-card" style={{ padding: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <select className="select-field" value={a} onChange={(event) => setA(event.target.value)}>{assets.map((item) => <option key={item.name}>{item.name}</option>)}</select>
            <select className="select-field" value={b} onChange={(event) => setB(event.target.value)}>{assets.map((item) => <option key={item.name}>{item.name}</option>)}</select>
          </div>
          ...
          <div className="glass-card" style={{ padding: 18 }}>
            <div className="table-wrap">
              <table className="data-table">
                ...
                <tbody>{ranking.map((item) => <tr key={item.name}><td>{item.name}</td><td>{item.type}</td><td>{format(item.price)}</td><td>{item.performance}%</td><td>{item.pe}</td><td>{item.benefit}</td></tr>)}</tbody>
              </table>
            </div>
          </div>
        </div>
